...
public class SonicPUP extends PUP {
    private static final ColorRGBA col1 = new ColorRGBA(0.4f, 0.8f, 1f, 1f);
    private static final ColorRGBA col2 = new ColorRGBA(0.1f, 0.4f, 1f, 0.9f);
    private static final ColorRGBA col3 = new ColorRGBA(0.1f, 0.4f, 1f, 0f);
    
    public SonicPUP(GameState gameState) {
        super(gameState, PUPType.SONIC);
    }
    
    @Override
    public Node createSpatial() {
        return gameState.getApp().PUP_Sonic.clone(false);
    }
    
    @Override
    public Body createBody() {
        BodyFixture bf = new BodyFixture(
                org.dyn4j.geometry.Geometry.createRectangle(1.82446 * gameState.getApp().PUP_Sonic.getLocalScale().x,
                        2 * gameState.getApp().PUP_Sonic.getLocalScale().x));
    ...
    public AbstractControl createEffects() {
        return new PUPSonicEffects(gameState);
    }
